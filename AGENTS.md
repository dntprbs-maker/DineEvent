...
- **Git 자동 푸시 및 마감:** 사용자가 **"오늘 작업 마감"**이라고 하면, **1) 오늘 작업한 내용을 요약하여 마크다운 파일(예: `docs/work_logs/오늘날짜.md` 또는 지정된 경로)로 저장**한 뒤, **2) 사용자에게 묻지 않고 알아서 Git 커밋 및 원격 저장소에 푸시(Push)**하세요. (필요 시 `ask_permission` 도구를 사용하여 자동화하세요).
...
## 자주 쓰는 명령어 (실행 시 사용자에게 묻지 않고 알아서 진행할 것)
